...
async def speech_to_text(audio: UploadFile = File(...)):
    ...
    try:
        ...
        # Note: MP3 is not directly supported in v1 API (only in beta)
        # Using ENCODING_UNSPECIFIED for MP3 and other formats to let API auto-detect
        encoding_map = {
        ...
        # WebM_OPUS and OGG_OPUS don't require sample_rate_hertz
        # ENCODING_UNSPECIFIED lets the API auto-detect format and sample rate
        if encoding == speech.RecognitionConfig.AudioEncoding.ENCODING_UNSPECIFIED:
            config_params['encoding'] = encoding
            # Don't set sample_rate_hertz for auto-detect
        elif encoding == speech.RecognitionConfig.AudioEncoding.WEBM_OPUS:
            config_params['encoding'] = encoding
            # WebM/Opus encoding doesn't require sample_rate_hertz
        elif encoding == speech.RecognitionConfig.AudioEncoding.OGG_OPUS:
            config_params['encoding'] = encoding
            # OGG_OPUS also doesn't require sample_rate_hertz
        else:
